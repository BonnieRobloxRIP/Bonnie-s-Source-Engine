...
# - Level title -
execute as @a[tag=easy] at @s if score r3_timer_s easy matches 21 if score r3_timer_ms easy matches 10 if score started3 easy matches 1 run title @a[tag=easy] subtitle Â§2The Wall
...
execute as @s[type=brr:flood_room, name=easy_room3] at @s if score R3B4 easy matches 0 if score R3B4a easy matches 10 if block ^-5 ^21 ^-6 brr:flood_button["brr:button_type"="exit","brr:pressed"=true, "minecraft:block_face"="down"] run tellraw @a[tag=easy] {"rawtext":[{"text": "Â§aTunnel Gate has been Opened!"}]}
