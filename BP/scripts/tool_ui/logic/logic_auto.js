...
import { ensureBlockDataArray, getNamedTargetEntriesSafe, buildRunOutputOptions } from "./shared_logic_helpers.js";
...
export function logicAutoUI(player, blockEntry, options) {
	...
	ensureBlockDataArray(blockEntry, "outputs");
	ensureBlockDataArray(blockEntry, "automations");

	const blockOutputs = [...blockEntry.data.outputs];
	const automations = [...blockEntry.data.automations];
	const namedTargetEntries = getNamedTargetEntriesSafe(getNamedTargetEntries);
	const runOutputOptions = buildRunOutputOptions(blockOutputs);
