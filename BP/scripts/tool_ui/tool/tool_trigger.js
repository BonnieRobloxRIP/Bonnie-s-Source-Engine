...
export function triggerToolUI(player, blockEntry, options) {
	...
	triggerForm.textField("Condition Value 1", "", { defaultValue: blockEntry.data?.conditionValue1 || "" });
	triggerForm.textField("Condition Value 2", "", { defaultValue: blockEntry.data?.conditionValue2 || "" });
	triggerForm.textField("Condition Value 3", "", { defaultValue: blockEntry.data?.conditionValue3 || "" });
	...
	triggerForm.textField("Output Name", "", { defaultValue: "" });
	...
	if (blockOutputs.length === 0) {
		...
		for (const output of blockOutputs) {
			...
			triggerForm.toggle("Delete", { defaultValue: false });
			triggerForm.divider();
		}
	}
	...
	triggerForm.show(player).then((response) => {
		...
		const selectedConditionRaw = formValues[cursor++];
		let selectedCondition = "noCondition";
		if (typeof selectedConditionRaw === "number") {
			selectedCondition = conditionTools[Number(selectedConditionRaw)] ?? "noCondition";
		} else {
			const selectedConditionText = `${selectedConditionRaw ?? "noCondition"}`.trim();
			selectedCondition = conditionTools.find(condition => condition.toLowerCase() === selectedConditionText.toLowerCase())
				?? (selectedConditionText || "noCondition");
		}
		...
		if (!conditionTools.includes(selectedCondition)) {
			sendUiError(player, `Unknown condition: ${selectedCondition}. Use a valid condition id such as noCondition.`);
			return;
		}
