...
const HEALTH_CHARGER_TYPE_ID = "brr:health_charger";
...
const EMPTY_ANIMATION_MAX_STAGE = 7;
const CHARGE_STEP = 5;
const CHARGE_INTERVAL_TICKS = 15;
const EMPTY_ANIMATION_STEP_TICKS = 1;
const EMPTY_ANIMATION_FIRST_HOLD_TICKS = 10;
const MAX_INTERACTION_DISTANCE = 3.5;
const START_SOUND = "interactable.health_charger.start";
const DENY_SOUND = "interactable.health_charger.deny";
...
function isStillCharging(session) {
    ...
    try {
        ...
        return block?.typeId === HEALTH_CHARGER_TYPE_ID
...
function startEmptyAnimation(block) {
    ...
    emptyAnimationDeadlines.set(blockKey, {
        ...
        stage: 0,
        nextStageTick: system.currentTick + EMPTY_ANIMATION_FIRST_HOLD_TICKS
    });
}

world.beforeEvents.playerInteractWithBlock.subscribe((event) => {
    if (event.cancel || event.block?.typeId !== HEALTH_CHARGER_TYPE_ID) return;
...
system.runInterval(() => {
    ...
    for (const [playerId, session] of activeSessions) {
        ...
        if (block?.typeId !== HEALTH_CHARGER_TYPE_ID) {
            endSession(playerId, true);
            continue;
        }

        // Charge draining is visual/audio only for now; no player healing is applied yet.
    ...
    for (const [blockKey, animation] of emptyAnimationDeadlines) {
        if (currentTick < animation.nextStageTick) continue;

        try {
            const dimension = world.getDimension(animation.dimensionId);
            const block = dimension.getBlock(animation.location);
            if (block?.typeId !== HEALTH_CHARGER_TYPE_ID || getChargeLevel(block) > 0) {
                emptyAnimationDeadlines.delete(blockKey);
                continue;
            }

            const nextStage = animation.stage + 1;
            setBlockState(block, EMPTY_ANIMATION_STATE, nextStage);
            if (nextStage >= EMPTY_ANIMATION_MAX_STAGE) {
                emptyAnimationDeadlines.delete(blockKey);
            } else {
                animation.stage = nextStage;
                animation.nextStageTick = currentTick + EMPTY_ANIMATION_STEP_TICKS;
            }
        } catch {
            emptyAnimationDeadlines.delete(blockKey);
        }
