...
import { flashShooterLight } from "./effects.js";
import {
    getCurrentTick,
...
function getLookTarget(shooter) {
    ...
    const entityHit = findEntityHit(shooter, viewOrigin, viewDirection, blockDistance, BEAM_HIT_RADIUS);
...
function applyBeamDamage(shooter, target, tick) {
    ...
    applyWeaponDamage(shooter, target, DAMAGE_PER_PULSE);
}
...
registerWeaponTick((players, tick, onlinePlayers) => {
    ...
    const hasActiveState = trackedUseByPlayer.size > 0 || activeFireStateByPlayer.size > 0;
    if (!hasActiveState && !players.some(p => isHoldingGluon(p))) return;
    ...
    for (const player of players) {
        ...
        const baseOrigin = getBeamOriginFromLaserPointApprox(
            player,
            lookTarget.viewDirection,
        ...
        const blockDistance = hasValidBlockHit
            ...
            : MAX_BEAM_DISTANCE;
        const hit = findEntityHit(player, beamOrigin, direction, blockDistance, BEAM_HIT_RADIUS);
        const beamLength = hit ? hit.distance : blockDistance;
        const hasBlockStop = !hit && hasValidBlockHit;
    ...
    pruneStalePlayerState([
        activeFireStateByPlayer,
        trackedUseByPlayer,
        nextFireLoopTickByPlayer,
        movementStateByPlayer,
        debugLastEventByPlayer,
        debugLastActionbarTickByPlayer
    ], onlinePlayers);
