...
function findAmmoInInventory(player) {
    try {
        ...
        for (let i = 0; i < container.size; i++) {
            ...
            if (!item || !MAGNUM_AMMO_ITEM_IDS.has(`${item.typeId ?? ""}`)) continue;
...
function canStartManualReload(player, intentTick = getCurrentTick()) {
    if (!player?.id || !isHoldingMagnum(player)) return false;

    const tick = getCurrentTick();
    const actionTick = Number.isFinite(intentTick) ? intentTick : tick;
    const heldTicks = Math.max(0, tick - actionTick);
    const reloadUntil = reloadUntilTickByPlayer.get(player.id) ?? -1;
    if (tick < reloadUntil) return false;

    const loadedRounds = getLoadedRounds(player);
    if (loadedRounds >= MAGNUM_MAG_SIZE) return false;
    if (getReserveAmmoTotal(player) <= 0) return false;

    // Reload only when intentionally crouch-holding the use button.
    if (!isSneaking(player)) return false;
    if (heldTicks < RELOAD_HOLD_THRESHOLD_TICKS) return false;
...
function finalizeRightClickUse(player) {
    ...
    if (canStartManualReload(player, startTick)) {
        tryReload(player);
        return;
    }
