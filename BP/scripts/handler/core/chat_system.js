...
const CHAT_SPAM_BLOCKED_MESSAGE_DEFAULT = "§cDon't you dare spam!";
const CHAT_SPAM_BLOCKED_MESSAGE_PROPERTY = "brr_chat_spam_blocked_message";
const PLAYER_USERNAMES_VISIBLE_PROPERTY = "player_usernames_visible";
let chatMessageCooldownMs = CHAT_MESSAGE_COOLDOWN_DEFAULT_MS;
let chatSpamBlockedMessage = CHAT_SPAM_BLOCKED_MESSAGE_DEFAULT;
...
export function executeMuteCommand(actor, targetInput, durationRaw, reasonRaw) {
...
export function executeUnmuteCommand(actor, targetInput, reasonRaw) {
    const reason = normalizeTextToken(reasonRaw);
    const targetPlayer = getPlayerFromCommandInput(targetInput);
...
function normalizeChatSpamBlockedMessage(rawMessage) {
    const normalized = `${rawMessage ?? ""}`
        .replaceAll("\r", " ")
        .replaceAll("\n", " ")
        .trim();

    return normalized || CHAT_SPAM_BLOCKED_MESSAGE_DEFAULT;
}
...
function applyChatSpamBlockedMessage(rawMessage, persist = true) {
    const nextMessage = normalizeChatSpamBlockedMessage(rawMessage);
    chatSpamBlockedMessage = nextMessage;

    if (!persist) return;

    try {
        const valueToStore = nextMessage === CHAT_SPAM_BLOCKED_MESSAGE_DEFAULT ? undefined : nextMessage;
        world.setDynamicProperty(CHAT_SPAM_BLOCKED_MESSAGE_PROPERTY, valueToStore);
    } catch {
        // Dynamic property can fail if called too early; runtime value still applies.
    }
}

function loadPersistedChatSpamBlockedMessage() {
    try {
        const storedValue = world.getDynamicProperty(CHAT_SPAM_BLOCKED_MESSAGE_PROPERTY);
        if (typeof storedValue === "string" && storedValue.trim().length > 0) {
            applyChatSpamBlockedMessage(storedValue, false);
            return;
        }
    } catch {
        // Keep default blocked message if dynamic properties are unavailable.
    }

    applyChatSpamBlockedMessage(CHAT_SPAM_BLOCKED_MESSAGE_DEFAULT, false);
}
...
export function getChatSpamBlockedMessage() {
    return chatSpamBlockedMessage;
}

export function setChatSpamBlockedMessage(rawMessage) {
    applyChatSpamBlockedMessage(rawMessage, true);
    return getChatSpamBlockedMessage();
}

system.run(() => {
    loadPersistedChatCooldown();
    loadPersistedChatSpamBlockedMessage();
    loadMuteState();
});
...
export function isDevNametagPlayer(playerOrName) {
...
function arePlayerUsernamesVisible() {
    try {
        return parseBooleanLike(world.getDynamicProperty(PLAYER_USERNAMES_VISIBLE_PROPERTY), true);
    } catch {
        return true;
    }
}
...
export function chatRank(player, tags) {
    ...
    if (["RicDevs"].includes(player.name)) {
        rankPrefix += "[§l§dContributor§r] ";
    }
    ...
    if (tags.includes("easy")) rankPrefix += "[§a§lEasy§r] ";
    if (tags.includes("medium")) rankPrefix += "[§e§lMedium§r] ";
    if (tags.includes("hard")) rankPrefix += "[§4§lHard§r] ";
    if (tags.includes("extreme")) rankPrefix += "[§5§lExtreme§r] ";
    if (tags.includes("endless")) rankPrefix += "[§9§lEndless§r] ";
...
export function handleMessage() {
    try {
        world.beforeEvents.chatSend.subscribe((data) => {
            ...
            if (chatMessageCooldownMs > 0 && now - lastMessageAt < chatMessageCooldownMs) {
                data.cancel = true;
                player.sendMessage(getChatSpamBlockedMessage());
                return;
            }
    ...
    system.runInterval(() => {
        ...
        const usernamesVisible = arePlayerUsernamesVisible();
